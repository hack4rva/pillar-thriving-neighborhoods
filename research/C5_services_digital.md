...
### Planning website organization: 2024-314 Site Plan transition and PDF forms
...
| :--- | :--- | :--- | :--- |
| **OPP (EnerGov)** | Public search available; API status unknown | High (Backend modifications risky) | **Build alongside**: Proxy searches to OPP; build address normalizer to bypass search bugs. |
...
### Unknowns to confirm
* **OPP API Access**: It is unknown if the EnerGov OPP offers a public API or if data must be scraped via HTML automation. Rate limits are also unknown.
...
| Site Plan ordinance (2024-314) replaced the Plan of Development (POD) process. | `https://www.rva.gov/planning-development-review/site-plan` |
